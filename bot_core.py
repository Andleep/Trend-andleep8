# bot_core.py  (محسّن: Lorentzian + MACD + Kelly sizing + commission/slippage + debug signals)
import pandas as pd
import numpy as np
import talib
import logging
...
class AdvancedLorentzianBot:
    ...
    def simulate(self, klines: pd.DataFrame, symbol: str, params: Dict) -> Dict:
        """
        Simulate trading on given klines DataFrame.
        Returns performance dict including debug info.
        """
        ...
        score_th = params.get('score_th', 1e-6)
        adx_th = params.get('adx_th', 5)
        ...
        for i in range(n):
            ...
            # check TP/SL intra-bar for existing positions
            ...
            # --- debug logging for score > threshold ---
            signal_checks += 1
            if score > score_th:
                debug_logs.append({
                    'index': i,
                    ...
                    'macdh': float(row.get('macdh', np.nan)),
                    'adx': float(row.get('adx', np.nan)),
                    'ema50': float(row.get('ema50', np.nan)),
                    'close': float(row['close'])
                })

            # --- modified (wider) entry condition: EMA or MACD + reasonable ADX ---
            ema_cond = row['close'] > row.get('ema50', row['close'])
            macd_cond = row.get('macdh', 0.0) > 0
            adx_cond = row.get('adx', 0.0) >= max(3, adx_th)  # allow minimum adx floor of 3

            is_up = (ema_cond or macd_cond) and adx_cond

            if score > score_th and is_up:
                buy_signal_count += 1

                # compute kelly fraction based on past wins/losses
                ...
                if trade_amt <= 0 or trade_amt > self.balance:
                    # cannot enter due to insufficient balance or constraints
                    self.equity_curve.append(self.balance)
                    continue
        ...
        # final close of any remaining positions at last price
        ...
        # debug summary to help diagnose why no trades occurred
        debug_summary = {
            'signal_checks': signal_checks,
            'buy_signals_detected': buy_signal_count,
            'buy_executed': buy_executed,
            'sample_score_hits': debug_logs[:20]  # up to 20 examples for inspection
        }
